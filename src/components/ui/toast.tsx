...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-4 left-4 right-4 z-[100] flex max-h-screen w-full flex-col-reverse p-4 sm:bottom-4 sm:right-4 sm:top-auto sm:flex-col md:max-w-[420px]",
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-xl border p-4 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border bg-white/80 backdrop-blur-md text-gray-900 shadow-[0_8px_30px_rgb(0,0,0,0.12)]",
        destructive: "border-red-200 bg-red-50/80 backdrop-blur-md text-red-900 shadow-[0_8px_30px_rgb(255,0,0,0.12)]",
        success: "border-green-200 bg-green-50/80 backdrop-blur-md text-green-900 shadow-[0_8px_30px_rgb(0,255,0,0.12)]",
        cart: "border-blue-200 bg-blue-50/80 backdrop-blur-md text-blue-900 shadow-[0_8px_30px_rgb(0,0,255,0.12)] hover:bg-blue-100/80 transition-colors",
      },
    },
    defaultVariants: {
      variant: "default",
